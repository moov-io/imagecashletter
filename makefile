...
client:
	@for cmd in mvn java; do \
		command -v $$cmd >/dev/null 2>&1 || { echo "make client requires '$$cmd' to be installed."; exit 1; }; \
	done
	@grep -rnwl './client/' -e 'Generated by: OpenAPI Generator' | xargs -n1 rm
	OPENAPI_GENERATOR_VERSION=4.3.1 ./openapi-generator generate -i openapi.yaml -g go -o ./client
