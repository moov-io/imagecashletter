...
func TestRead_multipleBundlesInCashLetter(t *testing.T) {
	r := NewReader(strings.NewReader(multipleBundles))
	f, err := r.Read()
	require.NoError(t, err)
	require.Equal(t, 2, len(f.CashLetters[0].Bundles))
	require.Equal(t, "1", f.CashLetters[0].Bundles[0].BundleHeader.BundleSequenceNumber)
...
const depositTicketAsCredit = `0135T123456789123456789201810101237NCitadel           Wells Fargo        US     
100123138010412104288220181010201810101237IGA1      Contact Name  5558675552    
62      123456789 031300012             5558881000000001000001              G101                    
200123138010412104288220181010201810109999      1   01                          
